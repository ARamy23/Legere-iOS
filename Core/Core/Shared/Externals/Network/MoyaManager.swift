...
public class MoyaManager: NetworkProtocol {
    static var ongoingRequests = [Cancellable]()
    
    public func callModel<T, U>(model: T.Type, api: U) -> Promise<T> where T : Decodable, T : Encodable, U : BaseTargetType {
...
struct NetworkErrorResponse: Codable {
    ...
    public func handleError(from code: Int) -> NetworkError {
        switch code {
        case 401:
